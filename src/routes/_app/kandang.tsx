...
function KandangPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const [farmRows, kandangRows] = await Promise.all([getFarms(), getKandang()]);
        if (!active) return;
        setFarms(farmRows as Farm[]);
        setKandang(kandangRows);
      } catch (error) {
        if (!active) return;
        const message = error instanceof Error ? error.message : "Data kandang belum bisa dimuat.";
  ...
  const save = async () => {
    ...
    if (!activeFarm) return toast.error("Buat data farm terlebih dahulu sebelum menambah kandang.");
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Data kandang gagal disimpan.";
      setErrorMessage(message);
      toast.error(message);
    } finally {
      setIsSaving(false);
      ...
      <PageHeader
        ...
        actions={
          ...
          >
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{edit.id ? "Edit" : "Tambah"} Kandang</DialogTitle>
              </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEdit({ ...edit, kapasitas: +e.target.value })}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      value={edit.pj}
                      onChange={(e) => setEdit({ ...edit, pj: e.target.value })}
                    />
                  </div>
                </div>
                <div className="grid gap-2">
                  ...
                  <Input
                    value={edit.lokasi}
                    onChange={(e) => setEdit({ ...edit, lokasi: e.target.value })}
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={edit.jenis}
                      onValueChange={(v) => setEdit({ ...edit, jenis: v as Kandang["jenis"] })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={edit.status}
                      onValueChange={(v) => setEdit({ ...edit, status: v as Kandang["status"] })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
