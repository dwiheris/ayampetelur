...
// TODO(supabase): Migrasikan modul ini ke tabel pakan dan pemakaian pakan.
const emptyP: Pakan = { id: "", nama: "", jenis: "Layer", stok: 0, minStok: 0, harga: 0 };
const emptyU: PemakaianPakan = {
  id: "",
...
function PakanPage() {
  ...
  const [editP, setEditP] = useState<Pakan>(emptyP);
  const [openU, setOpenU] = useState(false);
  const [editU, setEditU] = useState<PemakaianPakan>(emptyU);
  ...
  const saveU = () => {
    if (!editU.kandangId || !editU.pakanId || editU.jumlah <= 0)
  ...
  return (
    <>
      <PageHeader
        ...
        description="Master data pakan, stok, dan pemakaian harian."
      />
        ...
        <TabsContent value="stok" className="space-y-4">
          <div className="flex justify-end">
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editP.id ? "Edit" : "Tambah"} Pakan</DialogTitle>
                </DialogHeader>
                <div className="grid gap-3">
                  <div className="grid gap-2">
                    ...
                    <Input
                      value={editP.nama}
                      onChange={(e) => setEditP({ ...editP, nama: e.target.value })}
                    />
                  </div>
                  <div className="grid gap-2">
                    <Label>Jenis</Label>
                    <Input
                      value={editP.jenis}
                      onChange={(e) => setEditP({ ...editP, jenis: e.target.value })}
                    />
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Input
                        type="number"
                        value={editP.stok || ""}
                        onChange={(e) => setEditP({ ...editP, stok: +e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        type="number"
                        value={editP.minStok || ""}
                        onChange={(e) => setEditP({ ...editP, minStok: +e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        type="number"
                        value={editP.harga || ""}
                        onChange={(e) => setEditP({ ...editP, harga: +e.target.value })}
                      />
                    </div>
                  </div>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setOpenP(false)}>
                    Batal
                  </Button>
                  <Button onClick={saveP}>Simpan</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        ...
        <TabsContent value="pemakaian" className="space-y-4">
          <div className="flex justify-end">
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Input Pemakaian Pakan</DialogTitle>
                </DialogHeader>
                <div className="grid gap-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Input
                        type="date"
                        value={editU.tanggal}
                        onChange={(e) => setEditU({ ...editU, tanggal: e.target.value })}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        type="number"
                        value={editU.jumlah || ""}
                        onChange={(e) => setEditU({ ...editU, jumlah: +e.target.value })}
                      />
                    </div>
                  </div>
                  <div className="grid gap-2">
                    <Label>Kandang</Label>
                    <Select
                      value={editU.kandangId}
                      onValueChange={(v) => setEditU({ ...editU, kandangId: v })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      value={editU.pakanId}
                      onValueChange={(v) => setEditU({ ...editU, pakanId: v })}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Pilih" />
                      </SelectTrigger>
                      <SelectContent>
                        {pakan.map((p) => (
                          <SelectItem key={p.id} value={p.id}>
                            {p.nama} (stok {formatNumber(p.stok)} kg)
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                ...
                <DialogFooter>
                  <Button variant="outline" onClick={() => setOpenU(false)}>
                    Batal
                  </Button>
                  <Button onClick={saveU}>Simpan</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Per Ekor</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {pemakaian.length === 0 && (
                  ...
                  {[...pemakaian]
                    ...
                    .map((u) => (
                      <TableRow key={u.id}>
                        ...
                        <TableCell>
                          {kandang.find((k) => k.id === u.kandangId)?.nama ?? "—"}
                        </TableCell>
