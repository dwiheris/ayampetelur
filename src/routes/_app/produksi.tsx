...
const empty: Produksi = { id: "", tanggal: new Date().toISOString().slice(0, 10), kandangId: "", normal: 0, retak: 0, kecil: 0, jumbo: 0, berat: 0, catatan: "" };
...
function ProduksiPage() {
  ...
  const filtered = useMemo(() =>
    [...produksi].sort((a, b) => b.tanggal.localeCompare(a.tanggal))
      ...
      <PageHeader
        ...
        actions={
          <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEdit(empty); }}>
            <DialogTrigger asChild><Button><Plus className="mr-1 h-4 w-4" />Input Produksi</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Input Produksi Telur</DialogTitle></DialogHeader>
              <div className="grid gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="grid gap-2"><Label>Telur Normal</Label><Input type="number" value={edit.normal || ""} onChange={(e) => setEdit({ ...edit, normal: +e.target.value })} /></div>
                  <div className="grid gap-2"><Label>Telur Retak</Label><Input type="number" value={edit.retak || ""} onChange={(e) => setEdit({ ...edit, retak: +e.target.value })} /></div>
                  <div className="grid gap-2"><Label>Telur Kecil</Label><Input type="number" value={edit.kecil || ""} onChange={(e) => setEdit({ ...edit, kecil: +e.target.value })} /></div>
                  <div className="grid gap-2"><Label>Telur Jumbo</Label><Input type="number" value={edit.jumbo || ""} onChange={(e) => setEdit({ ...edit, jumbo: +e.target.value })} /></div>
                </div>
                <div className="grid gap-2"><Label>Berat Total (kg)</Label><Input type="number" step="0.1" value={edit.berat || ""} onChange={(e) => setEdit({ ...edit, berat: +e.target.value })} /></div>
                <div className="grid gap-2"><Label>Catatan</Label><Input value={edit.catatan ?? ""} onChange={(e) => setEdit({ ...edit, catatan: e.target.value })} /></div>
                <div className="rounded-md bg-muted px-3 py-2 text-sm">Total telur: <span className="font-semibold">{formatNumber(total)} butir</span></div>
              </div>
          ...
          <div className="flex flex-wrap gap-2">
            <Select value={filterKandang} onValueChange={setFilterKandang}>
              ...
              <SelectContent>
                ...
                {kandang.map((k) => <SelectItem key={k.id} value={k.id}>{k.nama}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow>
                <TableHead>Tanggal</TableHead><TableHead>Kandang</TableHead>
                <TableHead className="text-right">Normal</TableHead><TableHead className="text-right">Retak</TableHead>
                <TableHead className="text-right">Kecil</TableHead><TableHead className="text-right">Jumbo</TableHead>
                <TableHead className="text-right">Total</TableHead><TableHead className="text-right">Berat (kg)</TableHead>
                <TableHead className="text-right">HDP</TableHead><TableHead className="w-16">Aksi</TableHead>
                ...
                {filtered.slice(0, 100).map((p) => {
                  ...
                  return (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>{kandang.find(k => k.id === p.kandangId)?.nama ?? "—"}</TableCell>
