...
function StokTelurPage() {
  ...
  const riwayat = [
    ...produksi.map(p => ({
      tanggal: p.tanggal, jenis: "Masuk" as const, keterangan: `Produksi ${kandang.find(k => k.id === p.kandangId)?.nama ?? ""}`,
      jumlah: p.normal + p.retak + p.kecil + p.jumbo,
    })),
    ...penjualan.map(s => ({
      tanggal: s.tanggal, jenis: "Keluar" as const, keterangan: `Penjualan ${s.jenisTelur}`, jumlah: Math.round(s.jumlahKg * 16),
    })),
        ...
        {items.map((i) => (
          <Card key={i.label}>
            <CardContent className="flex items-center justify-between p-5">
              ...
              <div className={`flex h-11 w-11 items-center justify-center rounded-xl ${i.tone}`}><Egg className="h-5 w-5" /></div>
            </CardContent>
          </Card>
        ))}
      </div>
      <Card>
        <CardHeader><CardTitle>Riwayat Stok</CardTitle></CardHeader>
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Tanggal</TableHead><TableHead>Jenis</TableHead><TableHead>Keterangan</TableHead><TableHead className="text-right">Jumlah</TableHead>
            </TableRow></TableHeader>
            <TableBody>
              {riwayat.map((r, i) => (
                <TableRow key={i}>
                  <TableCell>{r.tanggal}</TableCell>
                  <TableCell className={r.jenis === "Masuk" ? "text-success font-medium" : "text-destructive font-medium"}>{r.jenis}</TableCell>
                  <TableCell>{r.keterangan}</TableCell>
                  <TableCell className="text-right">{r.jenis === "Masuk" ? "+" : "−"}{formatNumber(r.jumlah)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
